...
- MRK를 373행으로 정리하고 `0x1A` 바이트를 0개로 만들었습니다.
- 현재 ESP 대사에서 한 번도 나오지 않는 stale MRK keyword를 0개로 정리했습니다.
- `0x1A`가 들어가던 13개 토픽은 MRK에서 제거하고, 관련 INFO 응답 `NAME`을 직접 수정하여 실제 한국어 DIAL명이 대사에서 발견되도록 복구했습니다.
- RC5에서 `파르고스의 은닉처` 철자를 고친 뒤 남아 있던 옛 MRK override를 제거했습니다.
- 흐리스카르 `금화 되찾기` → `파르고스의 은닉처` → 선택지 흐름을 실게임에서 확인했습니다.
- 타베레 베드라노의 `그가 화내는` → `그가 화내는 걸 봤다` 링크를 실게임에서 확인했습니다.
- RC4 이후 ESP 변경은 검증된 INFO 응답 `NAME` 29개로 제한했고, 기술 서브레코드와 레코드 수는 유지했습니다.
- `CELL=0`, `PGRD=0`, `.top` 없음, 구형 `@ + 0x7F + #` 링크 없음, 대량 result-script `AddTopic` 주입 없음 상태를 유지합니다.
- 실제 RC6 pre-release를 재구축한 `release/rebuild_rc6.py`를 소스 저장소에 보존합니다.
